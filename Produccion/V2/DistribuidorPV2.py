...
ARQUITECTURA APPEND-ONLY (PV2):
- cola_WORKER_XXX.csv: Append-only por Distribuidor
- estados_WORKER_XXX.csv: Append-only por Worker
- Purga nocturna a las 00:01: Limpia comandos completados
"""
...
def parse_estados_file(estados_path: Path) -> Dict[str, int]:
    """
    Lee el archivo de estados y retorna un mapa: (ticketMaster_eventType) -> estado
    El último estado para cada key prevalece.
    """
    ...
    try:
        content = estados_path.read_text(encoding="utf-8", errors="replace")
        for line in content.splitlines():
            ...
            parts = line.split(";")
            if len(parts) >= 3:
                ticket_master = parts[0].strip()
                event_type = parts[1].strip()
                try:
                    estado = int(parts[2].strip())
                except ValueError:
                    continue
                key = f"{ticket_master}_{event_type}"
                estados_map[key] = estado  # Último estado prevalece
    except Exception as exc:
        print(f"[PURGA][ERROR] No se pudo leer estados {estados_path}: {exc}")
    
    return estados_map


def purga_nocturna(config: Config) -> None:
    """
    Purga nocturna: limpia comandos completados de cola y estados.
    
    Proceso:
    1. Para cada worker:
       a. Renombrar cola_WORKER_XXX.csv -> cola_WORKER_XXX.csv.lck
    ...
    for worker_id in config.worker_ids:
        ...
        try:
            # 1. Bloquear archivos (renombrar a .lck)
            if cola_path.exists():
                cola_path.rename(cola_lck)
            if estados_path.exists():
                estados_path.rename(estados_lck)
            
            print(f"[PURGA] Worker {worker_id}: Archivos bloqueados")
            
            # 2. Leer estados
            estados_map = parse_estados_file(estados_lck)
            
            # 3. Leer cola y filtrar
            comandos_pendientes: List[str] = []
            if cola_lck.exists():
                try:
                    cola_content = cola_lck.read_text(encoding="utf-8", errors="replace")
                    for line in cola_content.splitlines():
                        line_stripped = line.strip()
                        if not line_stripped:
                            continue
                        # Parsear línea para obtener ticket y event_type
                        parts = line_stripped.split(";")
                        if len(parts) >= 2:
                            event_type = parts[0].strip().upper()
                            ticket_master = parts[1].strip()
                            key = f"{ticket_master}_{event_type}"
                            
                            # Mantener si NO está completado (estado != 2)
                            if estados_map.get(key, -1) != 2:
                                comandos_pendientes.append(line_stripped)
                except Exception as exc:
            ...
            if estados_lck.exists():
                try:
                    ...
                    for line in estados_content.splitlines():
                        line_stripped = line.strip()
                        if not line_stripped:
                            continue
                        parts = line_stripped.split(";")
            ...
            with open(estados_path, "w", encoding="utf-8", newline="") as f:
            ...
            print(f"[PURGA] Worker {worker_id}: OK - Cola: {len(comandos_pendientes)} pendientes, Estados: {len(estados_activos)} activos")
            
        except Exception as exc:
            print(f"[PURGA][ERROR] Worker {worker_id}: {exc}")
            # Intentar restaurar archivos si hubo error
            try:
                if cola_lck.exists() and not cola_path.exists():
                    cola_lck.rename(cola_path)
                if estados_lck.exists() and not estados_path.exists():
                    estados_lck.rename(estados_path)
            except Exception:
                pass
    
    print(f"[PURGA] Purga nocturna completada")


def check_and_run_purge(config: Config) -> bool:
    """
    Verifica si es hora de ejecutar la purga nocturna (00:01).
    Retorna True si se ejecutó la purga.
    """
    now = datetime.now()
    if now.hour == PURGE_HOUR and now.minute == PURGE_MINUTE:
        purga_nocturna(config)
        return True
    return False


# Variable global para evitar múltiples purgas en el mismo minuto
_last_purge_date: Optional[str] = None


def maybe_run_purge(config: Config) -> None:
    """
    Ejecuta la purga nocturna si es el momento y no se ha ejecutado hoy.
    """
    ...
    # Si ya purgamos hoy, no hacer nada
    if _last_purge_date == today_str:
        return
    
    # Si es la hora de purga
    if now.hour == PURGE_HOUR and now.minute == PURGE_MINUTE:
        print(f"[PURGA] Hora de purga detectada: {now.strftime('%Y-%m-%d %H:%M:%S')}")
        purga_nocturna(config)
...
def run_service() -> None:
    cfg = get_config()
    print(f"[INIT] DistribuidorPV2 Spool arrancado.")
    ...
    print(f"[INIT] Purga nocturna programada para: {PURGE_HOUR:02d}:{PURGE_MINUTE:02d}")
    ...
    while True:
        try:
            ...
            # Verificar si es hora de purga nocturna
            maybe_run_purge(cfg)
